...
// Pending extractions stored in memory (tempId → data), 10 min TTL
...
export async function confirmBill(req: AuthRequest, res: Response): Promise<void> {
  ...
  const billType: BillType = confirmedData.type === 'ELECTRICITY' ? 'ELECTRICITY' : 'GAS';
  const co2Kg = toCo2(billType, confirmedData.usage);
  ...
  if (user.neighborhoodId) {
    ...
    if (neighborhoodAvg._count.id > 0 && neighborhoodAvg._avg.co2Kg !== null) {
      treesSaved = co2ToTrees(neighborhoodAvg._avg.co2Kg - co2Kg);
    }
  }
  ...
  const prevMonthTreesSaved = prevMonthCo2 !== null ? co2ToTrees(prevMonthCo2 - co2Kg) : null;
  ...
  const bill = await prisma.bill.upsert({
    ...
    update: {
      ...
      periodStart: new Date(confirmedData.periodStart),
      periodEnd: new Date(confirmedData.periodEnd),
      usage: confirmedData.usage,
      co2Kg,
      treesSaved,
      isConfirmed: true,
    },
