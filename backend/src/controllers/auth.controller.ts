 ...
 * SignOptions tip hatasını gidermek için as string cast işlemi eklenmiştir.
 */
...
export async function register(req: Request, res: Response): Promise<void> {
    try {
        ...
        try {
            ...
            // Prisma P2002: Unique constraint failed (E-posta zaten kayıtlı)
            if (err.code === 'P2002') {
...
export async function refresh(req: Request, res: Response): Promise<void> {
    try {
        ...
        // Token Rotation: Eski token'ı sil, yeni çifti oluştur
        ...
        await prisma.refreshToken.create({
            data: { userId: payload.id, token: newRefreshToken, expiresAt }
        });
